...
if ( $comments ) {
	...
	<div class="comments" id="comments">
		...
		<div class="comments-header section-inner small max-percentage">
			...
			<?php
			if ( ! have_comments() ) {
				_e( 'Be first in comment', 'ghuddi' );
			} elseif ( 1 === $comments_number ) {
				/* translators: %s: Post title. */
      printf( _x( 'One Comment', 'comments title', 'ghuddi' ), '' /*get_the_title()*/ );
			} else {
				printf(
          ...
          'echo'              => true
			...
			$comment_pagination = paginate_comments_links(
				array(
					...
					'end_size'  => 0,
					'mid_size'  => 0,
					'next_text' => __( 'Newer Comments', 'ghuddi' ) . ' <span aria-hidden="true">&rarr;</span>',
					'prev_text' => '<span aria-hidden="true">&larr;</span> ' . __( 'Older Comments', 'ghuddi' ),
				)
			);
			...
			if ( $comment_pagination ) {
				...
				<nav class="comments-pagination pagination<?php echo $pagination_classes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static output ?>" aria-label="<?php esc_attr_e( 'Comments', 'ghuddi' ); ?>">
...
if ( comments_open() || pings_open() ) {
  ...
  //Comment Form
	...
	<div class="comment-respond" id="respond">
		...
		<p class="comments-closed"><?php _e( 'Comments are closed.', 'ghuddi' ); ?></p>
